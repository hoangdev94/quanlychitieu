...
int totalBalance() {
  ...
  for (var i = 0; i < transactions.length; i++) {
    listAmount.add(transactions[i].type == 'Khoản thu'
...
List<Transaction> getExpenseTransactionToday() {
  ...
  for (var i = 0; i < transactions.length; i++) {
    if (transactions[i].category.type == 'Khoản chi' &&
...
String formatCurrency(int value) {
  ...
  final formattedValue = format.format(value).replaceAll('.', ',');
  return '${formattedValue}VND'; // Append ' vnđ' to the formatted value
}

String getFormattedDate(int index, DateTime selectedDate) {
  switch (index) {
    case 0:
      return DateFormat('dd MMM, yyyy', 'vi_VN').format(selectedDate.toLocal());
    case 1:
      final startOfWeek =
      selectedDate.subtract(Duration(days: selectedDate.weekday - 1));
      final endOfWeek = startOfWeek.add(const Duration(days: 6));
      return '${DateFormat('dd', 'vi_VN').format(startOfWeek)}-${DateFormat('dd', 'vi_VN').format(endOfWeek)} ${DateFormat('MMM, yyyy', 'vi_VN').format(selectedDate.toLocal())}';
    case 2:
      return DateFormat('MMM yyyy', 'vi_VN').format(selectedDate.toLocal());
    case 3:
      return DateFormat('yyyy', 'vi_VN').format(selectedDate.toLocal());
    default:
      return '';
  }
}